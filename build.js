/* ===== Build: Add shared header and footer placeholders =====
  Run: node build.js
  Pages load header.html and footer.html through js/layout.js. */
const fs = require('fs');
...
for (const file of files) {
  ...
  if (markerRe.test(html)) {
    ...
    html = html.replace(placeholderRe, headerPlaceholder);
  } else if (headerPlaceholderRe.test(html)) {
    html = html.replace(headerPlaceholderRe, headerPlaceholder);
  } else {
  ...
  html = html.replace(/\s*<script src="\.\.\/js\/(?:main|layout)\.js"><\/script>/, '\n  ' + layoutScript);
